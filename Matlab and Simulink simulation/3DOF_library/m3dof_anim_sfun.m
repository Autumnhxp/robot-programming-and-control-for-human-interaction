...
persistent p1 p2 p3 p4 p5 p6 p7 p8 p9 f fd fig_handle;
switch flag
    case 0
        sizes = simsizes;
        ...
        if isempty(get(0,'CurrentFigure'))
            %al = 2;
            fig_handle = figure('Resize','on','NumberTitle','on');
            hold on;
    ...
    case 3
        if not(isvalid(fig_handle))
            disp(['Figure closed for ' get_param(bdroot, 'Name') '.'])
            set_param(get_param(bdroot, 'Name'),'SimulationCommand','stop');
            return
        end
        figure(fig_handle)
